    ...
    <section className="container py-5">
      <div className="card card-bemc p-4">
        <div className="d-flex align-items-center gap-3 mb-3">
          {profile.avatarUrl ? <img src={profile.avatarUrl} alt="perfil" width="90" height="90" style={{ objectFit: 'cover', borderRadius: 12 }} /> : <div className="bg-light border rounded" style={{ width: 90, height: 90 }} />}
          <div>
        ...
        <div className="row g-3">
          ...
          <div className="col-md-6"><h2 className="h6">Áreas de experiencia</h2><p className="small mb-0">{(p.areasExperience || []).join(', ') || 'No registradas'}</p></div>
          <div className="col-md-6"><h2 className="h6">Servicios ofrecidos</h2><p className="small mb-0">{(p.servicesOffered || []).join(', ') || 'No registrados'}</p></div>
        ...
        <ul className="small">{(p.educationItems || []).map((ed, idx) => <li key={`e-${idx}`}>{ed.level}: {ed.title} - {ed.institution}</li>)}</ul>
